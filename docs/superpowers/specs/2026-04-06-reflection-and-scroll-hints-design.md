...
If any reflection line is missing, that field is empty string. Actions still parse normally. This means local-llm (9B) can skip reflection entirely and the agent still works, just without the self-evaluation benefit.
...
1. Reflection fields appear in agent output for local-coder and DeepSeek
2. local-llm (9B) either produces reflection or falls back to actions-only without breaking
